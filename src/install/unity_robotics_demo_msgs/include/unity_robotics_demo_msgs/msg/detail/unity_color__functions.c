  ...
  unity_robotics_demo_msgs__msg__UnityColor__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    unity_robotics_demo_msgs__msg__UnityColor * data =
      (unity_robotics_demo_msgs__msg__UnityColor *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!unity_robotics_demo_msgs__msg__UnityColor__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          unity_robotics_demo_msgs__msg__UnityColor__fini(&data[i]);
        }
        free(data);
        return false;
      }
