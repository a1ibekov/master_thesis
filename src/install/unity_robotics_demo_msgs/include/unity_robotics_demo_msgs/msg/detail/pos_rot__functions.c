  ...
  unity_robotics_demo_msgs__msg__PosRot__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    unity_robotics_demo_msgs__msg__PosRot * data =
      (unity_robotics_demo_msgs__msg__PosRot *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!unity_robotics_demo_msgs__msg__PosRot__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          unity_robotics_demo_msgs__msg__PosRot__fini(&data[i]);
        }
        free(data);
        return false;
      }
